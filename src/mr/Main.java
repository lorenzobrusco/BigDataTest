...
public class Main {
	...
	/**
	 * 
	 * The mapper split the init input and write in output the pair [key,index] and
	 * the time
	 *
	 */
	...
	static class GroupReducer extends Reducer<IntWritable, Text, Text, Text> {
		...
		protected void reduce(IntWritable key, Iterable<Text> values,
			...
			if (start) {
				...
				context.write(new Text(""), new Text(str.toString().substring(0, str.toString().length() - 1)));
			...
			context.write(new Text(""),
					new Text(str.toString().substring(0, str.toString().length() - 1)));
		}
	}
